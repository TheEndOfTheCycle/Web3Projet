    ...
    async function sendRequest() {
        ...
        try {
            ...
            if (result.status === 'success') {
                ajaxResponse.innerHTML = '<div class="success">' + result.message + '</div>';
                form.reset();
                document.querySelectorAll('.input').forEach(input => input.classList.remove('success', 'error'));

                // Redirection en cas de succès
                window.location.href = result.redirect;
            } else {
                ajaxResponse.innerHTML = '<div class="error">' + result.message + '</div>';
            }
        } catch (error) {
            console.error('Error:', error);
            document.querySelector('#ajax-response').innerHTML = '<div class="error">An error occurred. Please try again later.</div>';
        }
    }
});
