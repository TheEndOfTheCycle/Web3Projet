...
class Films extends PdoWrapper
{
    ...
    public function add_film_to_db($titre_film,$anSortie_film,$nom_real,$nom_affiche,$synopsis)//plusieurs_tags nous permet de determiner si tags est un tableau ou pas,de plus tags doit etre une instance de classe tag
    {
    ...
    public function remove_film_from_db($nomFilm)//cette fonction retire un film de la bde
    {
            $num_film=$this->getNumFilm($nomFilm);
            $para =["numF" =>$num_film];
            //il faut d abord enlever les references de ce films(les cles etrangeres) des tables concernes c a d jouer et et film_tag
            ...
            $this->exec($req,$para);
            //finalment on efface le film de films
            $para=["nomF" =>$nomFilm];
            $req="delete from Films where titre_film=:nomF";
            $this->exec($req,$para);
    }
    public function getMoviesByTagName($nomTag)//cette fonction retourne tous les Films ayant en tag le parametre de cette fonciton
    {
            $Otags = new Tags();
    ...
    public function addTagToFilm($nomFilm,$nomTag)//cette methode r ajoute un tag  au nomFilm
    {
        $Otags = new Tags();
